<%@ Page Title="" Language="C#" MasterPageFile="~/Pages/Content.master" AutoEventWireup="true" CodeFile="policy_detail.aspx.cs" Inherits="Pages_Flate_Rate_policy_detail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Toolbar" runat="Server">
    <ul class="toolbar">
        ...
        <li style="display: none;">
         ...
         <li>
            <input type="button" onclick="add_new();" id="btnAdd" style="background: url('../../App_Themes/functions/add.png') no-repeat; border: none; height: 40px; width: 90px;" />
        </li>
          <li>
            <input type="button" data-toggle="modal" data-target="#modal_policy_search" style="background: url('../../App_Themes/functions/search.png') no-repeat; border: none; height: 40px; width: 90px;" />
        </li>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Main" runat="Server">
    ...
    <style>
        ...
        .select_row {
            background-color: #94D1FF; /*#FACED7;*/
        }
    </style>
    <script>
        var policy_state='';
        ...
        var policy_loan_id='';//reserve to update record
        var loan_type;
        var loan_term_year;
        var loan_interest_rate;
        var loan_effective_date;
        var loan_amount;
        var product_type='';//1=Ordinary, 2=Mortgage, 3=Saving 
        ...
        $(document).ready(function () {
            ...
            //Loan controls
            loan_type = $('#<%=ddlLoanType.ClientID%>');
            loan_term_year= $('#<%=txtLoanTerm.ClientID%>');
            loan_interest_rate = $('#<%=txtInterest.ClientID%>');
            loan_effective_date = $('#<%=txtLoanEffective.ClientID%>');
            loan_amount = $('#<%=txtOutstandingLoan.ClientID%>');


            //policy_id.val('5162E90C-3694-47B4-B8E1-D1B6C82A842C');
            //get_all(policy_id.val());
            
            $('#tblContent *').attr('disabled','disabled');
            $('#btnSave').attr('disabled', 'disabled');
            policy_number.attr('disabled','disabled');
            $(".icon-search").hide();

            //event
            product.change(function(){
                get_pay_assured_year($(this).val());
                get_product_type($(this).val()); 
            ...
            app_date.datepicker().on('changeDate', function () {
                if(member_birth_date.val()!='')
                {
                    calculate_age(member_birth_date.val(), $(this).val());
                    member_age.val(customer_age);
                    // console.log('get age: '+customer_age);
                }
            });

            rider_birth_date.datepicker().on('changeDate',function(dob){
 
                calculate_age($(this).val(), app_date.val());
               
               
                rider_age.val(customer_age);
            });

            //
            var para_value =getParameterByName('policy_id');
            if(para_value !='' && para_value != null)
            {
                get_all(para_value);
            }
        ...
        //get channel item list
        function get_channel_item_list(channel_id, channel_item_id) //channel_item_id use for selected value
        {
            if(channel_id !='')
            {
                $.ajax({
                        ...
                        if (data.d.length > 0) {
                            ...
                            $.each(data.d, function (index, item) {
                                policy_company.append($('<option>', {
                                    ...
                                    value: item.Channel_Item_ID
                                }));
                            });
                            policy_company.val(channel_item_id);
                        } else {
                            policy_company.empty();
                            policy_company.append($('<option>', {
                                value: '',
        ...
        function get_product_type(product_id)
        {
            ...
            $.ajax({
                type: "POST",
                url: "../../PolicyWebService.asmx/GetProductType",
                ...
                success: function(data){
                    if(data.d!='')
                    {
                        ...
                        if(product_type !='2')// For Mortgage only
                        {
                            //disabled loan tab
                            $('#loan *').attr('disabled', 'disabled');
                        }
                        else
                        {
                            //enabled loan tab
                            $('#loan *').removeAttr('disabled');
                        }
        ...
        function clear_policy_controll()
        {
           // policy_id.val('');
            policy_number.val('');
            policy_customer.val('');
            ...
            policy_annual_origin_premium.val('');
            policy_extra_premium.val('');
            policy_extra_annual_premium.val('');
            policy_discount.val('');
            policy_total_premium_after_discount.val('');
            policy_effective_date.val('');
            policy_actual_premium.val('');
            policy_return_premium.val('');
            policy_payment_code.val('');
            policy_approved_date.val('');

            policy_owner_name.val('');
            policy_owner_gender.val('');
            policy_owner_birth_date.val('');

            $('#div_contact_list').html('');
        ...
        function permanent_disable()
        {
            ...
            member_age.prop('disabled',true);
            rider_age.prop('disabled',true);

            policy_number.attr('disabled','disabled');

            policy_premium.attr('disabled','disabled');
            policy_annaul_premium.attr('disabled','disabled');;
            policy_annual_origin_premium.attr('disabled','disabled');
            policy_extra_premium.attr('disabled','disabled');
            policy_extra_annual_premium.attr('disabled','disabled');
            policy_discount.attr('disabled','disabled');

            sale_agent_code.attr('disabled','disabled');
            sale_agent_name.attr('disabled', 'disabled');
        ...
        function add_new()
        {
            ...
            $('#policy_status').html( '<b><u> New</u></b>');
            $(".icon-search").show();
        ...
        function validate_policy()
        {
            ...
            else if(policy_actual_premium.val().trim()=='' || policy_actual_premium.val().trim()=='0')
            {
                return 'Actual premium is required.';
            }
            ...
            else if(sale_agent_code.val().trim()=='')
            {
                return 'Sale agent name is required.';
            }
            else if(product_type =='2')// is mortgage
            {
                if(loan_interest_rate.val().trim()=='' || loan_term_year.val().trim()=='' || loan_effective_date.val().trim()=='' || loan_amount.val().trim()=='')
                {
                    return 'Loan is quried.';
                }
                else
                {
                    return '';
        ...
        function save_policy()
        {
            ...
            else
            {
                ...
                if( policy_extra_annual_premium.val()=='')
                {
                    policy_extra_annual_premium.val(0);
                }
                if(policy_extra_premium.val()=='')
                {
                    policy_extra_premium.val(0);
                }
                ...
                if(policy_actual_premium.val()=='')
                {
                    policy_actual_premium.val(0);
                }
                if(policy_return_premium.val()=='')
                {
                    policy_return_premium.val(0);
                }
                ...
                policies[20] = sale_agent_code.val();
                //channel & company
                policies[21] = policy_channel.val();
                policies[22] = policy_company.val();
        ...
        function update_policy()
        {
            ...
            policies[21]=sale_agent_code.val();
            policies[22]=policy_channel.val();
            policies[23]=policy_company.val();
        ...
        function get_policy(policy_id)
        {
            $.ajax({
                ...
                success: function(data){
                   ...
                   // app_date.datepicker('update', app_date.val());// selected datetime picker
                    ...
                    //channel and company
                    policy_channel.val(policy.ChannelID);//select channel value
                   
                    //policy_company.val(policy.ChannelItemID);
                    GetCompany(policy.ChannelID);
                    //alert(policy_company.find('option').length);
                    get_channel_item_list(policy.ChannelID, policy.ChannelItemID);
                    //policy_company.val(policy.ChannelItemID);
                    
                    
                    $('#policy_status').html( '<b><u> ' + policy.PolicyStatus + '</u></b>');
                    //disabled controls.
                    if(policy.PolicyStatus!='New')
                    {
                        $('#tblContent *').attr('disabled','disabled');
                        $('#btnSave').attr('disabled', 'disabled');
                        policy_number.attr('disabled','disabled');
                    ...
                    //get policy owner contact
                    var str_contact='';
                    $.ajax({
                        type: "POST",
                        url: "../../CustomerWebService.asmx/GetContactList",
                        data:JSON.stringify({cust_id:policy.CustomerID}),
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function(data){
                            $.each(data.d,function(i,item){
                                str_contact += 'Name: ' + item.Name + '<br />Mobile: ' + item.Mobile + '<br /> Tel: ' + item.Tel + '<br /> Mail: ' + item.Mail + '<br /> Address: ' + item.Address + '<br />';
                            });
                            $('#div_contact_list').html(str_contact);
                        }
                    });

                    //sale agent

                    sale_agent_code.val(policy.SaleAgentID);
                    sale_agent_name.val(policy.SaleAgentName);

                    //get product type
                    get_product_type(policy.ProductID);
        ...
        function calc_policy_total_premium_after_discount()
        {
            ...
            var extra_premium=0;
            var discount =0;
            if(policy_premium.val()=='')
            {
                premium=0;
            }
            else
            {
                premium = parseInt(policy_premium.val());
            }
            if(policy_extra_premium.val()=='')
            {
                extra_premium=0;
            ...
            {
                extra_premium = parseInt(policy_extra_premium.val());
            }
            if(policy_discount.val()=='')
            {
                discount =0;
            ...
            {
                discount=parseInt(policy_discount.val());
            }
            ...
            policy_total_premium_after_discount.val(premium + extra_premium - discount);
        }

        var member_save_status=true;
        function save_member()
        {
            var tbl =$('#tbl_member_list tr:gt(0)');
            //alert(tbl.length);
            if(tbl.length>0)
            {
                var index=0;
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data.d)
                        {
                            ...
                            member_save_status =true;
                        }
                        else
                        {
                            member_save_status =false;
                        }
                    }
                      ...
                      success: function(data){
                          if(data.d)
                          {
                              if(member_id_delete!='')
                              {
                                  $.ajax({
                                      type: "POST",
                                      url: "../../PolicyWebService.asmx/DeleteMembers",
                                      data:JSON.stringify({member:member_id_delete}),
                                      contentType: "application/json; charset=utf-8",
                                      dataType: "json",
                                      async:false,
                                      success: function(data){
                ...
                success: function(data){
                    $.each(data.d, function(index, item){
                        ...
                        row = "<tr id='member_row_id" + member_row +  "'>" + 
                                ...
                                "<td style='display:none;'><lable id='lbl_member_no" + member_row  +  "'>" + member_row + "</label> </td>"+
                                "<td style='display:none;'><lable id='lbl_member_customer_id" + member_row +"'>" + item.CustomerID + "</lable></td>" +
                                "<td style='display:none;'><lable id='lbl_member_id" + member_row  +  "'>" + item.PolicyMemberID  +  "</label></td>" + 
        ...
        function search_policy() {
            ...
            var btn = $('#btnSearchPolicy');
            if (btn.val() == 'Search') {//first search

                var name = $('#<%=txtPolicySearchCustomerName.ClientID%>').val();
                var id_card = $('#<%=txtPolicySearchIDCard.ClientID%>').val();
                var gender = $('#<%=ddlPolicySearchGender.ClientID%>').val();
                var pol_number = $('#<%=txtPolicySearchPolicyNumber.ClientID%>').val();
                var app_number = $('#<%=txtPolicySearchAppNumber.ClientID%>').val();

                if (name == '' && id_card == '' && pol_number == '' && app_number == '' && gender==-1) {
                ...
                $.ajax({
                    type: "POST",
                    url: "../../PolicyWebService.asmx/GetSearchPolicyCustomer",
                    data: "{policy_number:'" + pol_number + "', app_number:'" + app_number + "', name:'" + name + "', gender:'" + gender + "', id_card:'" + id_card + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data) {
                        
                        if (data.d.length > 0) {
                            $('#div_policy_search_list').setTemplate($("#jTemplatePolicy").html());
                            $('#div_policy_search_list').processTemplate(data);

                           
                        }
                        else {
                            $('#div_policy_search_list').html('');
                        }
                    },
                    error: function (err)
                    {
                        alert('System got error: ' + err);
                    }
                ...
                get_policy(policy_id.val());
                get_all(policy_id.val());

                btn.val('Search');
                $('#modal_policy_search').modal('hide');
            }
        }
        ...
        function checked_policy(policy_id1, index, total_row) {
            if ($('#PolicyCh' + index).is(':checked')) {
                policy_id.val(policy_id1);
                
                // alert(customer_search_id);
                $('#Ch' + index).attr('background-color', 'red');

                $('#btnSearchPolicy').val('Ok');
            } else {
                policy_id.val('');
                $('#btnSearchPolicy').val('Search');
            }
            //Uncheck all other checkboxes
            for (var i = 0; i < total_row  ; i++) {
                if (i != index) {
                    $('#PolicyCh' + i).prop('checked', false);
                }
        ...
        function add_member()
        {
            ...
            if(member_extra_annual_premium.val()=='')
            {
                member_extra_annual_premium.val(0);
            }
            if(member_percentage.val()=='')
            {
                member_percentage.val(0);
            }
            if(member_rate.val()=='')
            {
                member_rate.val(0);
            ...
            else //add new member
            {
                ...
                row = "<tr id='member_row_id" + member_row +  "'>" + 
                        "<td style='display:none;'><lable id='lbl_member_no" + member_row  +  "'>" + member_row + "</label></td>" + 
                        "<td style='display:none;'><lable id='lbl_member_customer_id" + member_row  +  "'>" + member_customer_id.val() + "</label></td>" + 
                        "<td style='display:none;'><lable id='lbl_member_id" + member_row  +  "'></label></td>" +
        ...
        //clear all loan control
        function clear_loan_controls()
        {
            loan_type.val(0);
            loan_interest_rate.val('');
            loan_term_year.val('');
            loan_effective_date.val('');
            loan_amount.val('');
        }
        ...
        function clear_form()
        {
            clear_policy_controll(); 
            //clear member tab
            clear_member_controls();
            //clear cover tab
            clear_cover_controlls();
            //clear rider tab
            clear_rider_controlls();
            //clear beneficiary tab
            clear_beneficiary_controlls();
            //clear loan
            clear_loan_controls();
        ...
        function remove_member(row_index)
        {
            if (row_index > 0) {
                ...
                {
                    member_id_delete += $('#lbl_member_id' + row_index).text() + ";";
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data.d)
                        {
                            ...
                            //alert('Saved successfully.');
                            cover_save_status=true;
                        }
                        else
                        {
                            //alert('Saved fail.');
                            cover_save_status= false;
                        }
                    },
                    error: function (err){
        ...
        function process_grand_total()
        {
            var total_member_premium=0;
            var total_member_annual_origin_premium=0;
            var total_member_annual_premium=0;
            var total_member_extra_premium=0;
            var total_member_extra_annual_premium=0;
            var total_member_discount=0;

            var total_rider_premium=0;
            var total_rider_annual_origin_premium=0;
            var total_rider_annual_premium=0;
            var total_rider_extra_premium=0;
            var total_rider_extra_annual_premium=0;
            var total_rider_discount=0;

            var total_cover_premium=0;
            var total_cover_annual_premium=0;
            var total_cover_annual_origin_premium=0;

            var grand_total_premium=0;
            var grand_total_annual_premium=0;
            var grand_total_ananual_origin_premium=0;
            var grand_total_extra_premium=0;
            var grand_total_extra_annual_premium=0;
            var grand_total_discount=0;

            var tbl_cover = $('#tbl_cover_list tr:gt(0)');
            var tbl_rider =$('#tbl_rider_list tr:gt(0)');
            var tbl_member =$('#tbl_member_list tr:gt(0)');
            if(tbl_member.length>0)
            {
                tbl_member.each(function(index, element){
                    var td = $(element).find('td');
                    var i = td.eq(0).text();
                    total_member_premium += parseFloat( $('#lbl_member_premium_by_mode' + i).text());
                    total_member_annual_premium += parseFloat($('#lbl_member_annually_premium' +i).text());
                    total_member_annual_origin_premium += parseFloat($('#lbl_member_annual_origin_premium' +i).text());
                    total_member_extra_premium += parseFloat($('#lbl_member_extra_premium' +i).text());
                    total_member_extra_annual_premium += parseFloat($('#lbl_member_extra_annual_premium' +i).text());
                    total_member_discount += parseFloat($('#lbl_member_discount' +i).text());

                    //alert('Member annual origin:' + total_member_annual_origin_premium);
                });
              
            }
            if(tbl_cover.length>0)
            {
                tbl_cover.each(function(index, element){
                    var td = $(element).find('td');
                    var i = td.eq(0).text();
                    total_cover_premium += parseFloat( $('#lbl_cover_premium' + i).text());
                    total_cover_annual_premium += parseFloat( $('#lbl_cover_annual_premium' + i).text());
                    total_cover_annual_origin_premium +=parseFloat($('#lbl_cover_annual_origin_premium'+i).text());
                    // alert('Cover annual origin: ' + total_cover_annual_origin_premium);
                });
              
            }
            if(tbl_rider.length>0)
            {
                tbl_rider.each(function (index,element){
                    var td = $(element).find('td');
                    var i = td.eq(0).text();
                    total_rider_premium += parseFloat( $('#lblRiderPremiumByMode' + i).text());
                    total_rider_annual_origin_premium +=parseFloat($('#lblRiderAnnualOriginPremium'+i).text());
                    total_rider_annual_premium +=parseFloat($('#lblRiderAnnualPremium'+i).text());
                    total_rider_extra_premium += parseFloat($('#lblRiderExtraPremium'+i).text());
                    total_rider_extra_annual_premium +=parseFloat($('#lblRiderExtraAnnualPremium'+i).text());

                    // alert('Rider annual origin: ' + total_rider_annual_origin_premium);
                });
            }

            // alert('sub total: ' +( total_rider_premium + total_cover_premium));
            grand_total_premium= total_member_premium + total_cover_premium + total_rider_premium;
            grand_total_annual_premium = total_member_annual_premium + total_cover_annual_premium + total_rider_annual_premium;
            grand_total_ananual_origin_premium = total_member_annual_origin_premium + total_rider_annual_origin_premium + total_cover_annual_origin_premium;
            grand_total_extra_premium= total_member_extra_premium + total_rider_extra_premium;
            grand_total_extra_annual_premium = total_member_extra_annual_premium + total_rider_extra_premium;
            grand_total_discount = total_member_discount + total_rider_discount;

            policy_premium .val(grand_total_premium + grand_total_extra_premium);
            policy_annaul_premium.val(grand_total_annual_premium);
            policy_annual_origin_premium.val(grand_total_ananual_origin_premium);
            policy_extra_premium.val(grand_total_extra_premium);
            policy_extra_annual_premium.val(grand_total_extra_annual_premium);

            policy_discount.val(grand_total_discount);

            policy_total_premium_after_discount.val(grand_total_premium - grand_total_discount);
        ...
        {
            if(birth_date.trim()!='' && compare_date.trim()!='')
            {
        ...
        function select_rider(row_index)
        {
            ...
            //rider_birth_date.datepicker({dateFormat: "DD, d MM, yy"});
            //rider_birth_date.datepicker("setDate", $.rider_birth_date.parseDate( "yy-mm-dd", "2017-11-16" ));
            rider_birth_date.val($('#lblRiderBirthDate' + row_index).text());
            var my_date = rider_birth_date.val();
            rider_birth_date.datepicker({
                dateFormat: 'dd/mm/yy'
            });
            rider_birth_date.datepicker('update',new Date(my_date));
            ...
            selected_rider_premium=parseFloat(rider_premium_by_mode.val());
        ...
        function clear_rider_controlls()
        {
            clear_rider_boxes();
            $('#tbl_rider_list tr').each(function (index, element){
                $('#rider_row' + index).remove();
        ...
        function add_beneficiary()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    $('#lblBenFullName' + ben_selected_row).text(ben_kh_last_name.val() + ' ' + ben_kh_first_name.val( ));
        ...
        function clear_beneficiary_controlls()
        {
            $('#tbl_beneficiary_list tr').each(function (index,element){
                $('#ben_row' + index).remove();
            });
            ben_row_no=0;
            total_percentage=0;
            ben_delete_id='';
        ...
        //Get Loan
        function get_loan(policy_id)
        {
            $.ajax({
                type: "POST",
                url: "../../PolicyWebService.asmx/GetLoan",
                data:JSON.stringify({policyID:policy_id}),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function(data){
                    var loanObj = data.d;
                    if(loanObj.PolicyID=='' || loanObj.PolicyID==null)
                    {
                        loan_type.val(0);
                        loan_interest_rate.val('');
                        loan_term_year.val('');
                    ...
                    {
                        loan_type.val(loanObj.LoanType);
                        loan_interest_rate.val(loanObj.InterestRate);
                        loan_term_year.val(loanObj.TermYear);
                        loan_effective_date.val( formatJSONDate( loanObj.LoanEffectiveDate));
                        loan_amount.val(loanObj.Loan);
                        policy_loan_id = loanObj.PolicyID;
                    }
                }
            });
        }
        ...
        //Save Loan
        var loan_save_status=true;
        function save_loan()
        {
            var arr_loan = new Array();
            arr_loan[0] = loan_type.val();
            arr_loan[1]= loan_interest_rate.val();
            arr_loan[2] = loan_term_year.val();
            arr_loan[3] = loan_effective_date.val();
            arr_loan[4] = loan_amount.val();
           
            arr_loan[6] = policy_id.val();
            ...
            $.ajax({
                type: "POST",
                url: "../../PolicyWebService.asmx/SaveLoan",
                data:JSON.stringify({loan:arr_loan}),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                success: function(data){
                    if(data.d)
                    {
                        ...
                        loan_save_status =true;
                    }
                    else
                    {
                        loan_save_status =false;
        ...
        function save_all()
        {
            if(confirm('Confirm to save policy.!'))
            {
                ...
                if(policy_save_status ==true)
                {
                    save_member();
                    if(member_save_status!=true)
                    {
                        alert ('Saved member fail.');
                    ...
                    save_cover();
                    if(cover_save_status!=true)
                    {
                        alert ('Saved cover fail.');
                        if(policy_state=='NEW')
                        {
                            roll_back(policy_id.val());
                        }
                    ...
                    save_rider();
                    if(rider_save_status!=true)
                    {
                        alert ('Saved rider fail.');
                        if(policy_state=='NEW')
                        {
                            roll_back(policy_id.val());
                        }
                    ...
                    save_beneficiary();
                    if(ben_save_status!=true)
                    {
                        alert ('Saved beneficiary fail.');
                        if(policy_state=='NEW')
                        {
                            roll_back(policy_id.val());
                        }
                    ...
                    if(product_type=='2')//save loan if product type is mortgage
                    {
                        save_loan();
                        if(loan_save_status!=true)
                        {
                            alert ('Saved loan fail.');
                            if(policy_state=='NEW')
                            {
                                roll_back(policy_id.val());
                            }
        ...
        {
            alert('Rollback policy_id:' + policy_id);
            $.ajax({
                type: "POST",
    ...
    <script id="jTemplatePolicy" type="text/html">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th style="border-width: thin; border-style: solid;">No</th>
                    <th style="border-width: thin; border-style: solid;">Name</th>
                    <th style="border-width: thin; border-style: solid;">ID Card</th>
                    <th style="border-width: thin; border-style: solid;">Gender</th>
                    <th style="border-width: thin; border-style: solid;">Birth Date</th>
                    <th style="border-width: thin; border-style: solid;">Policy Number</th>
                    <th style="border-width: thin; border-style: solid; display: none;">Policy ID</th>
                    <th style="border-width: thin; border-style: solid; text-align: center; vertical-align: middle;">Select</th>
                </tr>
            </thead>
            <tbody>
                {#foreach $T.d as row}
                    <tr>
                        ...
                        {#/if}
                        <td style="vertical-align: middle;">{formatJSONDate($T.row.Birth_Date)}</td>
                        <td style="vertical-align: middle;">{$T.row.PolicyNumber}</td>
                        <td style="display: none;">{  $T.row.PolicyID }</td>
                        <td style="text-align: center; vertical-align: middle;">
                            <input id="PolicyCh{ $T.row$index }" type="checkbox" onclick="checked_policy('{ $T.row.PolicyID }', '{ $T.row$index }', '{ $T.row$total }');" /></td>
                    </tr>
                {#/for}
            </tbody>
        </table>
    ...
    <table id="tblContent" class="table-layout" style="background-color: #f6f6f6; padding: 40px;">
        <tr>
            ...
            <td class="td10 tdpading10">
                ...
                <span id="policy_status" class="required">*</span>
            </td>
        ...
        <tr>
            ...
            <td class="td10">
                <asp:TextBox ID="txtTermOfInsurance" onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                <span class="required">*</span>
            </td>
            <td class="td10">
            ...
            <td class="td10">
                <asp:TextBox ID="txtPaymentPeriod" onkeyup="ValidateNumber(this)"  runat="server"></asp:TextBox>
                <span class="required">*</span>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="td10">
                <asp:TextBox ID="txtPolicyPremium" onkeyup="ValidateNumber(this)"  runat="server"></asp:TextBox>
                USD
                <span class="required">*</span>
            </td>
            <td class="td10">
            ...
            <td class="td10">
                <asp:TextBox ID="txtPolicyAnnualOriginPremium" onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                USD
                <span class="required">*</span>
            </td>
        ...
        <tr>
            ...
            <td class="td10">
                <asp:TextBox ID="txtPolicyAnnualPremium" onkeyup="ValidateNumber(this)"  runat="server"></asp:TextBox>
                USD
                <span class="required">*</span>
            </td>
            <td class="td10">
            ...
            <td class="td10">
                <asp:TextBox ID="txtExtraPremium" onkeyup="ValidateNumber(this)"  runat="server"></asp:TextBox>
                USD
            </td>
        </tr>
        <tr>
            ...
            <td class="td10">
                <asp:TextBox ID="txtExtraAnnualPremium"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                USD
            </td>
            <td class="td10">
            ...
            <td class="td10">
                <asp:TextBox ID="txtPolicyDis"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="td10">
                <asp:TextBox ID="txtPolicyActualPremium"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                USD
            </td>
            <td class="td10">
            ...
            <td class="td10">
                <asp:TextBox ID="txtPolicyReturnPremium"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                USD
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="td10">
                <asp:TextBox ID="txtSaleAgentName" runat="server"></asp:TextBox>
            ...
            <td class="td10">
                <asp:TextBox ID="txtSaleAgentCode" runat="server"></asp:TextBox>
                    ...
                    <div class="container">
                        <div class="tab-content">
                            <div id="policy_owner" class="tab-pane active" style="padding-left: 10px;">
                                ...
                                <div id="div_contact" style=" border:1px solid #94D1FF; margin:10px; margin-right:20px;">
                                    <h4>Contact</h4>
                                    <div id="div_contact_list" style="margin-left:10px; padding-bottom:10px;">
                            ...
                            <div id="member" class="tab-pane" style="padding-left: 10px; padding-right: 10px;">
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberAge"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            <span class="required">*</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberSI" onkeyup="ValidateNumber(this)"  runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberPremiumByMode"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberAnnuallyOriginPremium"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberAnnuallyPremium" onkeyup="ValidateNumber(this)"  runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberExtraPercentage"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            %
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberExtraPremium"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberExtraAnnualPremium"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                                            USD
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtMemberDis"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                            ...
                            <div id="cover" class="tab-pane" style="padding-left: 10px; padding-right: 10px;">
                                <table>
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtCoverSI"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                        <td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtCoverPremiumByMode"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtCoverAnnualOriginPremium" onkeyup="ValidateTextDecimal(this)"  runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtCoverAnnualPremium"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                            ...
                            <div id="riders" class="tab-pane" style="padding-left: 10px; padding-right: 10px;">
                                <table>
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtRiderSI"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtRiderPremiumByMode"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtRiderAnnualOriginPremium"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtRiderAge"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            <span class="required">*</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtRiderAnnualPremium"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtRiderExtraPremium"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            USD
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtRiderExtraAnnualPremium"  onkeyup="ValidateTextDecimal(this)" runat="server"></asp:TextBox>
                                            USD
                            ...
                            <div id="beneficiary" class="tab-pane" style="padding-left: 10px; padding-right: 10px;">
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtBenPercentage"  onkeyup="ValidateNumber(this)" runat="server"></asp:TextBox>
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    <tr>
                            ...
                            <div id="loan" class="tab-pane" style="padding-left: 10px; padding-right: 10px;">
                                <table>
                                    <tr>
                                        <td>Loan Type</td>
                                        <td>
                                    ...
                                    <tr>
                                        <td>Interest</td>
                                        <td>
                                            <asp:TextBox ID="txtInterest" runat="server"></asp:TextBox><span> %</span>
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Term</td>
                                        <td>
                                            <asp:TextBox ID="txtLoanTerm" runat="server"></asp:TextBox><span> Year</span>
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Loan Effective</td>
                                        <td>
                                            <asp:TextBox ID="txtLoanEffective" runat="server" CssClass ="datepicker"></asp:TextBox>
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <asp:TextBox ID="txtOutstandingLoan" runat="server"></asp:TextBox>
                                            <span> USD</span>
                                            <span class="required">*</span>
                                        </td>
                                    </tr>
    ...
    <div id="modal_policy_search" class="modal hide fade large" tabindex="-1" role="dialog" aria-labelledby="SearchCustomer" aria-hidden="true" data-keyboard="false" data-backdrop="static">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            <h3 id="H1">Search Policy</h3>
        </div>
        <div class="modal-body ">
            <ul id="Ul2" class="nav nav-tabs" data-tabs="tabs">
                <li class="active"><a href="#search_policy" data-toggle="tab" style="text-decoration: none; font-size: 9pt;">Policy Number</a></li>
                <li><a href="#search_policy_by_customer_name" data-toggle="tab" style="text-decoration: none; font-size: 9pt;">Customer Info.</a></li>
            </ul>
            <div id="Div2" class="tab-content" style="height: 70px; overflow: hidden;">

                <div id="search_policy" class="tab-pane active">
                    <table style="width: 70%;">
                        <tr>
                            <td>
                                <asp:Label runat="server" ID="Label1" Text="Policy Number"></asp:Label>
                            </td>
                            <td>
                                <asp:TextBox runat="server" ID="txtPolicySearchPolicyNumber" Width="100px"></asp:TextBox>
                            </td>
                             <td>
                                <asp:Label runat="server" ID="Label5" Text="Application Number"></asp:Label>
                            </td>
                            <td>
                                <asp:TextBox runat="server" ID="txtPolicySearchAppNumber" Width="100px"></asp:TextBox>
                            </td>
                        </tr>
                    </table>
                </div>
                <div id="search_policy_by_customer_name" class="tab-pane">
                    <table style="width: 100%;">
                        <tr>
                           
                            <td>
                                <asp:Label runat="server" ID="Label3" Text="Customer Name"></asp:Label>
                            </td>
                            <td>
                                <asp:TextBox runat="server" ID="txtPolicySearchCustomerName"></asp:TextBox>
                            </td>
                            <td>
                                <asp:Label runat="server" ID="Label2" Text="Gender"></asp:Label>
                            </td>
                            <td>
                                <asp:DropDownList runat="server" ID="ddlPolicySearchGender" Width="90px">
                            ...
                            <td>
                                <asp:Label runat="server" ID="Label4" Text="ID Card"></asp:Label>
                            </td>
                            <td>
                                <asp:TextBox runat="server" ID="txtPolicySearchIDCard"></asp:TextBox>
                            </td>
                        </tr>
            ...
            <div id="div_policy_search_list"></div>
        </div>
        <div class="modal-footer">
            <input type="button" id="btnSearchPolicy" value="Search" onclick="search_policy();" class="btn btn-primary" />
        </div>
    </div>
